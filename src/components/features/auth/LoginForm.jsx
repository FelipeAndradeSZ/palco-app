...
export default function LoginForm({ onSubmit, loading, error, returnTo = '/' }) {
  ...
  const handleOAuth = async (provider) => {
    ...
    try {
      const { error: oauthFailure } = await signInWithOAuth(provider, returnTo);
      if (oauthFailure) throw oauthFailure;
    } catch (oauthFailure) {
      setOauthError(oauthFailure.message || 'Nao foi possivel entrar com este provedor.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      {/* Social Login */}
      {oauthError && <Alert type="error" message={oauthError} />}
      <div className={`grid gap-3 ${appleAuthEnabled ? 'grid-cols-2' : 'grid-cols-1'}`}>
        <Button 
          type="button" 
          variant="secondary" 
        ...
        >
          ...
          {oauthLoading === 'google' ? 'Abrindo...' : 'Google'}
        </Button>
        {appleAuthEnabled && (
          <Button
            type="button"
          ...
          >
            ...
            {oauthLoading === 'apple' ? 'Abrindo...' : 'Apple'}
          </Button>
        )}
      </div>
      ...
      <div className="relative flex items-center py-2">
        ...
        <span className="flex-shrink-0 mx-4 text-palco-text-subtle text-xs uppercase">Ou entre com email</span>
        <div className="flex-grow border-t border-palco-border"></div>
      </div>
